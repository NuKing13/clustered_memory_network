...
def extract_mean(states):
    ...
    for i in range(N):
        zmstates[:, i : i + 1] = states[:, i : i + 1] - scipy.mean(
            states[:, i : i + 1]
        )
    return zmstates
    ...
    for i in scipy.arange(variables):
        ...
        if pos == 0:
            ...
            task = taskfun(
                powerlist[i],
                scipy.concatenate(
                    (scipy.zeros((pos, inp.shape[1])), inp[:-pos, :])
                ),
            )
        output = output * task
    return output
...
def cov_capacity(
    states, target, return_all_scores=False, R_inv=None
):  # ,zm_target=True,zm_states=True):
    ...
    if not (R_inv.any()):
        R_inv, crank = scipy.linalg.pinv2(
...
class capacity_iterator:
    ...
    def collect(self, inputs, states):
        ...
        while donext:
            ...
            score = cov_capacity(
                estates[self.delay + self.window - 2 :, :],
                outputs[self.delay + self.window - 2 :, :],
                R_inv=self.corrmat,
            )

            p_threshold = (
                self.orth_factor
                * chi2.isf(1 - self.p_threshold, self.dimensions)
                / samples
            )
        ...
        if self.verbose >= -1:
            tag = "Total capacity=%(score).3f (%(perc).2f percent)" % {
                ...
                "perc": 100.0 * self.totalscore() / estates.shape[1],
            }
            print(tag)
    ...
    def score(self, score, threshold):
        if not (self.use_scores):
            print(
                "WARNING: scoring has been switched off, any further scores will be ignored"
            )
        if self.scored:
            print(
                "WARNING: duplicate scoring of individual, any further scores will be ignored"
            )
        self.scored = True
        ...
        if self.current_score > self.leaky_score:
            ...
            self.leaky_score = (
                1.0 - self.score_leak
            ) * self.leaky_score + self.score_leak * score
    ...
    def next(self):
        ...
        if not (self.scored) and self.use_scores:
            ...
            print(
                "Warning: current task has not been scored -- scoring is incomplete and has been switched off"
            )
        self.scored = False
        ...
        if (
            self.delay <= self.delskip
            or not (self.use_scores)
            or not (self.monotonous_delay)
            or (
                self.use_scores
                and self.monotonous_delay
                and (
                    self.current_score > 0.0
                    or (
                        self.delay > self.delskip
                        and self.current_score <= 0.0
                        and self.holding < self.delhold
                    )
                )
            )
        ):
        ...
        if (
            ...
            if (
                ...
                and (self.delay_score == 0.0)
                and (self.variables > 2)
            ):
                maxpos = scipy.arange(
                    self.window - self.variables + 1, self.window - 1, 1.0
                )
                    ...
                    if self.positions[index + 1] < maxpos[index]:
                        ...
                        if index == 0:
                            ...
                            for index2 in scipy.arange(index - 1, -1.0, -1.0):
                                if (
                                    self.positions[index2 + 1]
                                    < self.positions[index2 + 2] - 1
                                ):
                                    break
        ...
        if (
            not (self.use_scores)
            or not (self.monotonous_powerlist)
            or (
                self.use_scores
        ...
        if (
            not (self.use_scores)
            or not (self.monotonous_variables)
            or (
                self.use_scores
        ...
        if (
            not (self.use_scores)
            or not (self.monotonous_degree)
            or (
                self.use_scores
                and self.monotonous_degree
                and (
                    (self.variables_score >= self.d_threshold)
                    or (self.prev_var_score >= self.d_threshold)
                )
            )
        ):
            print("sixth if")
